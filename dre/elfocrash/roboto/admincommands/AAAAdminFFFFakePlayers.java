...
public class AAAAdminFFFFakePlayers implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		if(command.startsWith(ADMIN_COMMANDS[4])) {
			...
			String subStr = command.substring(ADMIN_COMMANDS[4].length()).trim();//16 yra "admin_deletefake" simbolių kiekis
			...
			if(subStr.length() <= 0)//tikriname ar komanda neturi papildomu parametru
			{
				...
				{
					//po komandos įvestas raktinis žodis "all"
		...
		if (command.startsWith(ADMIN_COMMANDS[3])) {
			...
			boolean matchesHtm = subStr.matches(".*[hH][tT][mM].*");//perrasau komanda į regex
			if(matchesHtm)
				subStr = subStr.replaceFirst(" [hH][tT][mM]", "");//nutrinam jei netycia 
	...
	private static void kazkaVeikiam(String subStr, Player activeChar, boolean spawn) {
		...
		{
			if(spawn)
				FFFFakePlayerManager.INSTANCE.spawnPlayers(fakeCount, fakeRadius, activeChar.getX(), activeChar.getY(), activeChar.getZ(), classDesired);
			else
