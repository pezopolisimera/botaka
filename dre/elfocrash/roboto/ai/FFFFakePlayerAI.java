...
public abstract class FFFFakePlayerAI
{
	...
	public FFFFakePlayerAI(FFFFakePlayer character)
	{
		...
		_spawnLocation = new Location(_ffffakePlayer.getX(), _ffffakePlayer.getY(), _ffffakePlayer.getZ());
		setup();
		applyDefaultBuffs();
	}
		...
		for (Creature npc : nearby) {
		    if (!npc.isDead() && npc.getTarget() == _ffffakePlayer) {
		        if (npc instanceof Player) {
		            Player p = (Player) npc;
		            if (p.isGM()) continue;
		            if (p.getPvpFlag() == 0 && p.getKarma() == 0) continue;
		        }
	...
	// Μεταβλητές για έξυπνη AI
	private long _lastCombatTime = System.currentTimeMillis();
	private long _currentTargetLockTime = 0;
	private final int MAX_IDLE_MILLIS = 180_000;
	private final int MAX_TARGET_LOCK_TIME = 5000; 
	private final int ATTACK_RADIUS = 1000;
	private final int RETURN_RADIUS = 1500;

	protected Location _spawnLocation;
	
	private boolean _targetJustSet = false;
	public void advancedThinkAndAct() {
	    if (_ffffakePlayer.isDead())
	        return;

	    Creature target = (_ffffakePlayer.getTarget() instanceof Creature) ? (Creature) _ffffakePlayer.getTarget() : null;

	    if (target != null && !target.isDead()) {
	        if (!_ffffakePlayer.isCastingNow()) {
	            if (System.currentTimeMillis() - _currentTargetLockTime > MAX_TARGET_LOCK_TIME) {
	                _ffffakePlayer.setTarget(null);
	                _currentTargetLockTime = 0;
	            }
	        } else {
	            _lastCombatTime = System.currentTimeMillis();
	            _currentTargetLockTime = System.currentTimeMillis();
	        }
	    }

	    if (System.currentTimeMillis() - _lastCombatTime > MAX_IDLE_MILLIS) {
	        moveTo(_spawnLocation.getX(), _spawnLocation.getY(), _spawnLocation.getZ());
	        return;
	    }

	    if (_ffffakePlayer.getTarget() == null) {
	        tryTargetAttackerOrRandomMob(Creature.class, ATTACK_RADIUS);
	        _targetJustSet = true;
	        return;
	    }
	    _targetJustSet = false;	    

	     if (!_ffffakePlayer.isInsideRadius(_spawnLocation.getX(), _spawnLocation.getY(), _spawnLocation.getZ(), RETURN_RADIUS, true, false)) {
	        moveTo(_spawnLocation.getX(), _spawnLocation.getY(), _spawnLocation.getZ());
	    }
	}
}
