 ...
 *
 */
public class FFFakePlayer extends Player
{
	private FFFakePlayerAI _fffakeAi;	
	...
	protected FFFakePlayer(int objectId)
	{
		super(objectId);
	}
	
	public FFFakePlayer(int objectId, PlayerTemplate template, String accountName, PcAppearance app)
	{
		super(objectId, template, accountName, app);
	}
		
	public FFFakePlayerAI getFFFakeAi()
	{
		return _fffakeAi;
	}
	
	public void setFFFakeAi(FFFakePlayerAI _fffakeAi)
	{
		this._fffakeAi = _fffakeAi;
	}
	
	public void assignDefaultAI() {
		try {
			setFFFakeAi(FFFakeHelpers.getAIbyClassId(getClassId()).getConstructor(FFFakePlayer.class).newInstance(this));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
