 ...
 *
 */
public class FFakePlayer extends Player
{
	private FFakePlayerAI _ffakeAi;	
	...
	protected FFakePlayer(int objectId)
	{
		super(objectId);
	}
	
	public FFakePlayer(int objectId, PlayerTemplate template, String accountName, PcAppearance app)
	{
		super(objectId, template, accountName, app);
	}
		
	public FFakePlayerAI getFFakeAi()
	{
		return _ffakeAi;
	}
	
	public void setFFakeAi(FFakePlayerAI _ffakeAi)
	{
		this._ffakeAi = _ffakeAi;
	}
	
	public void assignDefaultAI() {
		try {
			setFFakeAi(FFakeHelpers.getAIbyClassId(getClassId()).getConstructor(FFakePlayer.class).newInstance(this));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
