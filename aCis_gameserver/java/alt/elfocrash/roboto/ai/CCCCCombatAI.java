...
public abstract class CCCCCombatAI extends FFFFFakePlayerAI {
	...
	protected void selfSupportBuffs() {
	    List<Integer> activeEffects = new java.util.ArrayList<>();
	    ...
	    for (SSSSSupportSpell selfBuff : getSelfSupportSpells()) {
	        ...
	        L2Skill skill = SkillTable.getInstance().getInfo(
	            selfBuff.getSkillId(), 
	            _fffffakePlayer.getSkillLevel(selfBuff.getSkillId())
	        );

	        if (skill == null || !_fffffakePlayer.checkUseMagicConditions(skill, true, false))
	            continue;
