...
public abstract class FFFFFakePlayerAI
{
	...
	protected void applyDefaultBuffs() {
	    for (int[] buff : getBuffs()) {
	        try {
	            L2Effect[] effectsArray = _fffffakePlayer.getAllEffects();
	            Map<Integer, L2Effect> activeEffects = new java.util.HashMap<>();

	            if (effectsArray != null) {
	                for (L2Effect e : effectsArray) {
	                    if (e != null && e.getSkill() != null && e.getEffectType() == L2EffectType.BUFF) {
	                        int skillId = e.getSkill().getId();
	                        // Φυλάμε μόνο τον πρώτο που βρίσκουμε (ή μπορείς να διαλέξεις με βάση duration κλπ)
	                        if (!activeEffects.containsKey(skillId)) {
	                            activeEffects.put(skillId, e);
	                        }
	                    }
	                }
	            }
	            ...
	            if (!activeEffects.containsKey(buff[0])) {
	                SkillTable.getInstance().getInfo(buff[0], buff[1])
	                    .getEffects(_fffffakePlayer, _fffffakePlayer);
	            } else {
