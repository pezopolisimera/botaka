 ...
 *
 */
public class FFFFFakePlayer extends Player
{
	private FFFFFakePlayerAI _fffffakeAi;	
	...
	protected FFFFFakePlayer(int objectId)
	{
		super(objectId);
	}
	
	public FFFFFakePlayer(int objectId, PlayerTemplate template, String accountName, PcAppearance app)
	{
		super(objectId, template, accountName, app);
	}
		
	public FFFFFakePlayerAI getFFFFFakeAi()
	{
		return _fffffakeAi;
	}
	
	public void setFFFFFakeAi(FFFFFakePlayerAI _fffffakeAi)
	{
		this._fffffakeAi = _fffffakeAi;
	}
	
	public void assignDefaultAI() {
		try {
			setFFFFFakeAi(FFFFFakeHelpers.getAIbyClassId(getClassId()).getConstructor(FFFFFakePlayer.class).newInstance(this));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
