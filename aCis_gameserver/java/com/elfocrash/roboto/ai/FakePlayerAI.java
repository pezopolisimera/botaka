...
public abstract class FakePlayerAI
{
	...
	protected void tryTargetRandomCreatureByTypeInRadius(Class<? extends Creature> creatureClass, int radius)
	{
		if(_fakePlayer.getTarget() == null) {
			List<Creature> targets = _fakePlayer.getKnownTypeInRadius(creatureClass, radius).stream().filter(x->!x.isDead()
					/*
					 * cia atsifiltruosim zaidejus kurie turi karmos arba flagai
					 */
					&& x instanceof Player
					&& (((Player)x).getPvpFlag() != 0 || ((Player)x).getKarma() > 0)
					&& !Creature.isInsidePeaceZone(_fakePlayer, x)//ar ne peace zonoj zinoma :?
					&& ((Player) x).getAllyId() != _fakePlayer.getAllyId()
					&& !((Player) x).isGM()
			...
			if ( ((Creature)_fakePlayer.getTarget()).isDead() ||
				(_fakePlayer.getTarget() instanceof Player ? ((Player) _fakePlayer.getTarget()).getPvpFlag() <= 0 : false) )
			_fakePlayer.setTarget(null);
		}	
	}	
	...
	protected void toVillageOnDeath() {
		...
		_fakePlayer.getFakeAi().teleportToLocation(location.getX(), location.getY(), location.getZ(), 0);
	}
