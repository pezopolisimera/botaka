...
import compvp.elfocrash.roboto.ai.CCombatAI;
import compvp.elfocrash.roboto.model.HHealingSpell;

import net.sf.l2j.gameserver.model.actor.Creature;

public interface IIHealer {
	
	default void tryTargetingLowestHpTargetInRadius(FFakePlayer player, Class<? extends Creature> creatureClass, int radius) {
	...
	default void tryHealingTarget(FFakePlayer player) {
		
		if(player.getTarget() != null && player.getTarget() instanceof Creature)
		{
			Creature target = (Creature) player.getTarget();
			if(player.getFFakeAi() instanceof CCombatAI) {
				HHealingSpell skill = ((CCombatAI)player.getFFakeAi()).getRandomAvaiableHealingSpellForTarget();
				if(skill != null) {
					switch(skill.getCondition()){
						case LESSHPPERCENT:
							...
							if(currentHpPercentage <= skill.getConditionValue()) {
								player.getFFakeAi().castSpell(player.getSkill(skill.getSkillId()));						
							}						
							break;				
						default:
							break;							
