...
public class EEEnchanterAI extends FFFakePlayerAI {
	...
	public EEEnchanterAI(FFFakePlayer character) {
		super(character);
	}
	...
	public void setup() {
		super.setup();
		ItemInstance weapon = _fffakePlayer.getActiveWeaponInstance();		
		weapon = checkIfWeaponIsExistsEquipped(weapon);		
		weapon.setEnchantLevel(0);
		_fffakePlayer.broadcastCharInfo();
	...
	private void destroyFailedItem(ItemInstance weapon) {
		_fffakePlayer.getInventory().destroyItem("Enchant", weapon, _fffakePlayer, null);
		_fffakePlayer.broadcastUserInfo();
		_fffakePlayer.setActiveEnchantItem(null);
	}
	...
	private ItemInstance checkIfWeaponIsExistsEquipped(ItemInstance weapon) {
		if(weapon == null) {
			FFFakeHelpers.giveWeaponsByClass(_fffakePlayer, false);
			weapon = _fffakePlayer.getActiveWeaponInstance();
		}
		return weapon;
	}
